...
def encode_splits(data, split_data):
    ...
    for split, idxs in split_data.items():
        if split == "vg_coco_train":
            split_new = "train"
        elif split == "vg_coco_val":
            split_new = "val"
        elif split == "vg_coco_test":
            split_new = "test"
        else:
            print("no such split")
        for idx in idxs:
            id_to_split[idx] = split_new

    split_data_keys = ["train", "val", "test"]
    split_dict = {k:list() for k in split_data_keys}
...
def filter_images(data, split_data, image_data, all_image_ids):
    ...
    for img in data:
        ...
        # img['regions']: [{},{}]
        if keep:
            idx = all_image_ids.index(img['id'])
            img_h = image_data[idx]['height']
            img_w = image_data[idx]['width']
            img_new = {}
            img_region_new = []
            for i in range(len(img['regions'])):
                region_w = img['regions'][i]['width']
                region_h = img['regions'][i]['height']
                x1 = img['regions'][i]['x']
                y1 = img['regions'][i]['y']
                x2 = x1 + img['regions'][i]['width']
                y2 = y1 + img['regions'][i]['height']
                ph = img['regions'][i]['phrase']
                keep2 = x1>=0 and y1>=0 and region_h>0 and region_w>0 and x2<=img_w and y2<=img_h and len(ph)>0
                if keep2:
                    img_region_new.append(img['regions'][i])
                else:
                    num_invalid_box += 1
            img_new['regions'] = img_region_new
            img_new['id'] = img['id']
            new_data.append(img_new)
    print('num invalid region:', num_invalid_box)
    return new_data
...
def main(args):
    ...
    data = filter_images(data, split_data, image_data, all_image_ids)  # data为vg数据集定义的region description的格式的列表
...
if __name__ == '__main__':
    ...
    parser.add_argument('--region_data',
                        default='/home/ubuntu/D/hyj/data/visual-genome-v1.2/region_descriptions_v1.2.json',
                        help='Input JSON file with regions and captions')
    parser.add_argument('--image_data',
                        default='/home/ubuntu/D/hyj/data/visual-genome-v1.2/image_data_v1.2.json',
                        help='Input JSON file with image url weight and height')
    parser.add_argument('--split_json',
                        default='/home/ubuntu/D/hyj/data/visual-genome-v1.2/densecap-split/vg_coco_splits.json',
                        help='JSON file of splits')

    # OUTPUT settings
    parser.add_argument('--pickle_output',
                        default='/home/ubuntu/D/hyj/project/densecap-pytorch-main/data/data_new/VGCOCO-regions-dicts-lite.pkl',
                        help='Path to output pickle file')
    parser.add_argument('--h5_output',
                        default='/home/ubuntu/D/hyj/project/densecap-pytorch-main/data/data_new/VGCOCO-regions-lite.h5',
