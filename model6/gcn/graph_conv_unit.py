...
class _Collection_Unit(nn.Module):
    def __init__(self, dim, gcn_dim, use_bn=False):
    ...
    def forward(self, source, attention_base):
...
class _GraphConvolutionLayer_Collect(nn.Module):
    ...
    def __init__(self, node_dim, rel_dim, gcn_dim, use_bn=False):
        super(_GraphConvolutionLayer_Collect, self).__init__()
        self.collect_units = nn.ModuleList()
        self.collect_units.append(_Collection_Unit(rel_dim, gcn_dim, use_bn=use_bn)) # obj (subject) from rel
        self.collect_units.append(_Collection_Unit(rel_dim, gcn_dim, use_bn=use_bn)) # obj (object) from rel
        self.collect_units.append(_Collection_Unit(node_dim, gcn_dim, use_bn=use_bn)) # rel from obj (subject)
        self.collect_units.append(_Collection_Unit(node_dim, gcn_dim, use_bn=use_bn)) # rel from obj (object)
