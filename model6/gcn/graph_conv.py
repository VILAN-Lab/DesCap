...
class _GraphConvolutionLayer(nn.Module):
    def __init__(self, node_dim=2048, rel_dim=1024, gcn_dim=1024, use_bn=False):
        super(_GraphConvolutionLayer, self).__init__()
        self.gcn_collect = _GraphConvolutionLayer_Collect(node_dim, rel_dim, gcn_dim, use_bn=use_bn)
    ...
    def forward(self, feat_obj, feat_rel, map_obj_rel):
        ...
        source_rel_sub = self.gcn_collect(feat_rel, map_sobj_rel, 0)
        source_rel_obj = self.gcn_collect(feat_rel, map_oobj_rel, 1)
        ...
        source_obj_sub = self.gcn_collect(feat_obj, map_rel_sobj, 2)
        source_obj_obj = self.gcn_collect(feat_obj, map_rel_oobj, 3)
