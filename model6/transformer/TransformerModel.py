...
class TransformerModel(CaptionModel):
    ...
    def __init__(self, vocab_size, pad_idx=0, start_idx=1, end_idx=2):
        ...
        self.model = self.make_model(tgt_vocab=tgt_vocab)
    ...
    def _prepare_feature(self, att_feats, att_masks=None, seq=None):
        ...
        att_feats = pack_wrapper(self.att_embed, att_feats, att_masks)  # 维度变换到d_model=512
        # att_feats:(64, 6, 512)

        if att_masks is None:
            att_masks = att_feats.new_ones(att_feats.shape[:2], dtype=torch.long) # (64,6)
        att_masks = att_masks.unsqueeze(-2)  # (64,1,6)
    ...
    def _forward(self, att_feats, seq, seq_len, att_masks=None):
        """
        (64,6,1024) / (64,17) / np.array:(64,)
        """
