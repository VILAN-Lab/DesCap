...
class BoxDescriber(nn.Module):
    ...
    def forward_train(self, feat, cap_gt, cap_lens):
        ...
        :param feat: (batch_size, feat_size)     # (512, 4096)
        :param cap_gt: (batch_size, max_len)     # (512, 16)
        :param cap_lens: (batch_size,)           # (512)
    ...
    def forward(self, feat, cap_gt=None, cap_lens=None, box=None):

        if isinstance(cap_gt, list) and isinstance(cap_lens, list):
            cap_gt = torch.cat(cap_gt, dim=0)  # torch.Size([512, 17])
            cap_lens = torch.cat(cap_lens, dim=0)  # torch.Size([512])
        ...
        if self.training:
            ...
            cap_lens = torch.clamp(cap_lens - 1, min=0)  # xianzhifanwei
            # cap_lens is remain the <bos> and <eos>, so need to -1
