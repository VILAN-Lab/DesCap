...
from utils.box_utils import BoxRelationalEmbedding, get_sub_sg, generate_sg_matrix_box
...
def caption_loss(caption_predicts, caption_gt, caption_length):
    ...
    Arguments:
        ...
        caption_loss (Tensor)
    """
    # caption_predicts: torch.Size([256, 16, 10629])  '<eos>' does not include in input(predict)

    if isinstance(caption_gt, list) and isinstance(caption_length, list):
        caption_gt = torch.cat(caption_gt, dim=0)  # (batch_size, max_len+1) --> (512, 17)
    ...
    # '<bos>' does not include in target
...
class DenseCapRoIHeads(nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):  # 每张图片循环
            ...
            if gt_boxes_in_image.numel() == 0:
                ...
                match_quality_matrix = box_ops.box_iou(gt_boxes_in_image, proposals_in_image)  # (45, 2000+45)
                # iou (Tensor[N, M]): the NxM matrix containing the IoU values for every element in boxes1 and boxes2

                matched_idxs_in_image = self.proposal_matcher(match_quality_matrix)  # (2045)

                clamped_matched_idxs_in_image = matched_idxs_in_image.clamp(min=0)

                labels_in_image = gt_labels_in_image[clamped_matched_idxs_in_image]  # device = cuda

                # Label background (below the low threshold)
                bg_inds = matched_idxs_in_image == self.proposal_matcher.BELOW_LOW_THRESHOLD  # device = cuda
                # 2045中为-1的位置为true

                labels_in_image[bg_inds] = torch.tensor(0).to(device)

                # Label ignore proposals (between low and high thresholds)
                ignore_inds = matched_idxs_in_image == self.proposal_matcher.BETWEEN_THRESHOLDS
                labels_in_image[ignore_inds] = torch.tensor(-1).to(device) # -1 is ignored by sampler
    ...
    def select_training_samples(self, proposals, targets):
        ...
        sampled_inds = self.subsample(labels)  # 128
        matched_gt_boxes = []
    ...
    def forward(self, features, proposals, image_shapes, targets=None):
        ...
        box_features = self.box_roi_pool(features, proposals, image_shapes)  # torch.Size([2048, 256, 7, 7]) --> (boxes_num,C,output_size,output_size)
        box_features = self.box_head(box_features)  # torch.Size([128, 4096]) --> (boxes_num, box_feat_size)
        # eval(): box_features: (2000, 4096)
        logits, box_regression = self.box_predictor(box_features)  # torch.Size([2048, 2]) torch.Size([2048, 8])
        if self.training:
            ...
            # keep_ids: [tensor0[512],tensor1[512],tensor2[512],tensor3[512]] ,tensor element is bool
            boxes_per_image = [boxes_in_image.shape[0] for boxes_in_image in proposals]  # [512, 512, 512, 512]
            box_features = box_features.split(boxes_per_image, 0)  # (torch.Size([512, 4096]),torch.Size([512, 4096]),torch.Size([512, 4096]),torch.Size([512, 4096]))
            box_features_gt = []
            boxes_gt = []  # add
            for i in range(len(keep_ids)):  # i = 0,1,2,3
                box_features_gt.append(box_features[i][keep_ids[i]])
                boxes_gt.append(proposals[i][keep_ids[i]])  # add
                caption_gt[i] = caption_gt[i][keep_ids[i]]
                caption_length[i] = caption_length[i][keep_ids[i]]

            box_features = torch.cat(box_features_gt, 0)  # torch.Size([64, 2048])
            region_box = torch.cat(boxes_gt, 0)
            caps = torch.cat(caption_gt, 0)
            caps_len = torch.cat(caption_length, 0)

            # ----------- gcn --------------#
            pad_subg_feat, pad_subg_box, subg_node_num = self.gcn(box_features, region_box, targets)
            # ----------- transformer --------------#
            caption_predicts = self.box_describer(
                box_features,
                region_box,
                caption_gt[0],
                caption_length[0],
            ...
            # (64,2048)  (64,4)  (64,17)  (64,)  (64,7,1024)  (64,7,1024)  list:[] len=64
            # out: (64,16,10629)
        else:
            boxes_per_image = [boxes_in_image.shape[0] for boxes_in_image in proposals]  # [1000, 1000]
            box_features = box_features.split(boxes_per_image, 0)  # (torch.Size([1000, 4096]),torch.Size([1000, 4096]))
            box_features_gt = []
            boxes_gt = []  # add
            for i in range(len(boxes_per_image)):  # i = 0,1
                box_features_gt.append(box_features[i])
                boxes_gt.append(proposals[i])  # add
            box_features = torch.cat(box_features_gt, 0)  # torch.Size([2000, 4096])
            region_box = torch.cat(boxes_gt, 0)
            
            pad_subg_feat, pad_subg_box, subg_node_num = self.gcn(box_features, region_box, targets)
            seq = self.box_describer(
                box_features,
                region_box,
                pad_subg_feat,
                pad_subg_box,
            ...
            # seq: (300,16)
            caption_predicts = torch.ones(seq.size(0), seq.size(1)+2, dtype=torch.long) * 1  # 1 is the '<bos>'
            caption_predicts[:, -1] = 2  # 2 is '<eos>'
            caption_predicts[:, 1:-1] = seq

        
        # eval(): torch.Size([300, 17]) is the word index / train(): torch.Size([64, 16, 10629])

        result, losses = [], {}
        if self.training:
            loss_classifier, loss_box_reg = detect_loss(logits, box_regression, labels, regression_targets)
            loss_caption = caption_loss(caption_predicts, caption_gt, caption_length)  # linear+log_softmax --> cross_entropy
            #loss_caption2 = caption_loss(caption_predicts2, caption_gt, caption_length) # linear --> cross_entropy
            #assert loss_caption == loss_caption2
